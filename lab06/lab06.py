...
class Cat:
    ...
    def adopt_a_cat(cls, owner):
        ...
        n_lives = len(owner) + len(name)
        return cls(name, owner, n_lives)
...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        count = 0
        every_y_balance = self.balance
        while amount - every_y_balance > 0:
            every_y_balance = every_y_balance * (1 + self.interest)
            count += 1
        return count
...
run_docstring_examples(NoisyCat, globals(), True)
# run_docstring_examples(, globals(), True)
