...
def merge(incr_a, incr_b):
    ...
    while next_a != None or next_b != None:
        if next_a == None or next_b != None and next_b < next_a:
            yield next_b
            next_b = next(iter_b, None)
        elif next_b == None or next_a != None and next_a < next_b:
            yield next_a
...
class PairsIterator:
    ...
    def __next__(self):
        ...
        if self.i == len(self.lst):
            raise StopIteration
        ret = (self.lst[self.i], self.lst[self.j])
...
def flip_two(s):
    ...
    "*** YOUR CODE HERE ***"
    while s is Link.empty or s.rest is Link.empty:
        s.first, s.rest.first = s.rest.first, s.first
        s = s.rest.rest
