...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 0:
            r_num0 = strategy0(score0, score1)
        #take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE)
            score0 += take_turn(r_num0, score0, score1, dice, goal)
            score0 += hog_pile(score0, score1)
        else:
            r_num1 = strategy1(score1, score0)
            score1 += take_turn(r_num1, score1, score0, dice, goal)
            score1 += hog_pile(score1, score0)
        who = next_player(who)
    # END PROBLEM 5
    # (note that the indentation for the problem 7 prompt (***YOUR CODE HERE***) might be misleading)
    # BEGIN PROBLEM 7
        leader, message_say = say(score0, score1, leader)
        if message_say != None:
            print(message_say)
    # END PROBLEM 7
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        ...
        s = None
    message = f"Player {s} takes the lead by {bigger}"
    if s == None or (last_leader != None and s == last_leader):
        message = None
...
def make_averaged(original_function, total_samples=1000):
    ...
    def func1(*argc):
        ...
        while i < total_samples:
            i, ret = i + 1, ret + original_function(*argc)
        return ret / total_samples
    return func1
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    ret = 1
    while i < 11:
        total_num = make_averaged(roll_dice, total_samples)(i, dice)
...
def hefty_hogs_strategy(score, opponent_score, threshold=10, num_rolls=6):
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    stra = hefty_hogs_strategy(score, opponent_score, threshold, num_rolls)
    # take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE)
    tmp = take_turn(0, score, opponent_score)
    # hog_pile(player_score, opponent_score)
    return 0 if hog_pile(tmp + score, opponent_score) != 0 \
        or stra == 0 else num_rolls
    # END PROBLEM 11
