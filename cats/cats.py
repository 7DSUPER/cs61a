...
def choose(paragraphs, select, k):
    ...
    while i < len(paragraphs):
        if select(paragraphs[i]):
            paragraphs1 += [paragraphs[i]]
        i += 1
    if k >= len(paragraphs1):
        return ''
    return paragraphs1[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    # return func_4about
    topic = [lower(x) for x in topic]
    def func_4about(paragraphs):
        paragraphs = split(remove_punctuation(paragraphs))
        paragraphs = [lower(x) for x in paragraphs]
        for tp in topic:
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    count, i = 0, 0
    l_ty, l_re = len(typed_words), len(reference_words)
    if l_ty == 0 and l_re == 0:
        return 100.0
    if l_ty == 0 and l_re != 0:
        return 0.0
    for i in range(l_ty):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    typed_word = lower(typed_word)
    word_list = [lower(x) for x in word_list]
    ret_str = ''
    tmp = 0
    min = sys.maxsize
    for wl in word_list:
        if wl == typed_word:
            return wl
        tmp = diff_function(typed_word, wl, limit)
        if tmp > limit:
            continue
        if min > tmp:
            min = tmp
...
def minimum_mewtations(start, goal, limit):
    ...
    if start[0] == goal[0]:  # Fill in the condition
        ...
        return min( \
            minimum_mewtations(goal[0] + start, goal, limit - 1), \
                minimum_mewtations(start[1:], goal, limit - 1), \
                minimum_mewtations(start[1:], goal[1:], limit - 1), \
                ) + 1
        # END
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    faster_l = [[] for i in player_indices]
    # for i in player_indices:
    #     faster_l.append(sum(match["times"][i][j] for j in range(len(match["times"][i]))))
    for j in word_indices:
        tmp = [time(match, i, j) for i in player_indices]
        print("DEBUG: times:", tmp)
        print("DEBUG: index:",tmp.index(min(tmp)))
        faster_l[tmp.index(min(tmp))].append(word_at(match, j))
    return faster_l
    # END PROBLEM 10
