...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        self.health -= amount
        for ele in self.place.bees[:]:
            ele.reduce_health(amount)
        if self.health <= 0:
            for ele in self.place.bees[:]:
                ele.reduce_health(self.damage)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_waterproof:
            # if self.ant:
            #     self.ant.reduce_health(self.ant.health)
            # elif self.bees:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while cur_place:
            if cur_place.ant:
                if not cur_place.ant.is_buffer:
                    cur_place.ant.double()
                if cur_place.ant.is_container and cur_place.ant.ant_contained != None:
                    if not cur_place.ant.ant_contained.is_buffer:
